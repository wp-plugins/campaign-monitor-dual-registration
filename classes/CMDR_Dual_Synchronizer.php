...
class CMDR_Dual_Synchronizer {
	...
	// Single user update
	public static function cmdr_user_update( $user_id, $user_email = null, $new_user = false ) {
		...
		if ( $cmdr_api_key && $cmdr_list_id ) {
			...
			if ( $new_user ) {
				...
				if ( ! empty( $cmdr_settings[ 'user_role' ] ) && ! empty( $user->roles[ 0 ] ) ) {
					$args[ 'CustomFields' ][] = array( 'Key' => '_cmdr_wp_user_role', 'Value' => $user->roles[ 0 ] );
				}
			} else {
				$result = $wrap_s->get( $user->user_email );
				...
				if ( $subscriber && $subscriber->State == 'Active' ) {
					...
					if ( ! empty( $cmdr_settings[ 'user_role' ] ) && ! empty( $user->roles[ 0 ] ) ) {
						if ( empty( $custom_values[ '_cmdr_wp_user_role' ] ) || trim( $custom_values[ '_cmdr_wp_user_role' ] ) != trim( $user->roles[ 0 ] ) ) {
							$args[ 'CustomFields' ][] = array( 'Key' => '_cmdr_wp_user_role', 'Value' => $user->roles[ 0 ] );
						}
					}
				}
	...
	public static function cmdr_mass_update() {
		...
		if ( $cmdr_api_key && $cmdr_list_id ) {
			...
			if ( ! empty( $cmdr_settings[ 'user_role' ] ) ) {
				$missing_fields[] = '_cmdr_wp_user_role';
			}
